...
A dedicated cover image uploaded for a category or album (see Admin panel below)
is stored separately under `covers/categories/` or `covers/albums/`, with the same
`thumb_` prefix convention, named independently of the source file (a random
filename) since it has no association with any album folder.
...
- **Plugins** (`admin/plugins.php`, gated on `site_configuration`) — lists every discovered "feature" plugin (see [Plugins](#plugins) below) with an Enable/Disable toggle. Disabled by default; disabling a plugin stops it running immediately without deleting its data. A disabled plugin can be **permanently deleted** — a per-row Delete button, plus checkbox-based bulk selection with a "Delete Selected" toolbar button (same select-all/count/AJAX pattern as the Images page's bulk actions) — which removes its entire folder under `plugins/` from disk; an enabled plugin can never be deleted from either path, and anything already gone by the time a bulk request lands is silently skipped rather than erroring.
...
- **Appearance** — a visual theme card grid (screenshot, name, author, an **Active** badge on the current theme, **Activate**/**Preview**/**Details** actions on every card) replacing the old plain theme dropdown; a **Details** modal per theme shows its full screenshot gallery, author, design URI, and folder name; **install a theme from a `.zip`** upload (validated: real ZIP structure, entry-count and uncompressed-size caps, unsafe-path rejection, a `template.html` present in the archive, a single wrapping top-level folder auto-flattened); **update an already-installed theme in place** by re-uploading a `.zip` for its folder name, with an explicit overwrite confirmation and files staged to a temp folder first so a bad upload can never leave a half-extracted theme live; **delete** any non-active, non-bundled installed theme; the Default Colour Mode, Category Layout, and "Powered by" credit settings that previously lived on the Configuration page's Appearance card
...
### Themes
Themes live in `themes/{name}/` and require only `template.html`. The active theme is selected in Admin → Appearance. Multiple themes can be installed simultaneously.
...
Appearance sets the site-wide fallback for first-time visitors.
...
Recognized fields are `Theme Name`, `Author`, and `Design URI`. When present, they're shown on the theme's card and details modal in Admin → Appearance. The header is entirely optional — themes without one still work normally, falling back to the folder name.

Screenshot discovery follows the same convention: a file named `preview.*`, `thumbnail.*`, or `screenshot.*` (jpg/jpeg/png/webp/avif) in a theme's folder becomes its card thumbnail, in that priority order; numbered variants (`screenshot-2.jpg`, `screenshot-3.webp`, …) are picked up too and shown together in the details modal's screenshot gallery. A theme with no screenshot file gets a neutral placeholder card image instead of a broken image icon.

**Admin-only theme preview.** A logged-in administrator can append `?theme=folder-name` to any public gallery URL — the home page, a category page, or an album page — to render that theme for the current request only; the site's configured theme is never changed and no other visitor is affected. An unknown or invalid folder name falls back to the real active theme with an admin-only notice explaining why; a valid preview shows a small admin-only banner as a reminder it's temporary. The preview follows you as you keep browsing: every nav link, breadcrumb, category/album card, pagination link, and sort control generated while a preview is active carries the `theme` parameter forward, so navigating between categories and albums stays on the previewed theme for the whole session instead of reverting after one click. Every theme card in Admin → Appearance includes a **Preview** button that opens this URL in a new tab.
